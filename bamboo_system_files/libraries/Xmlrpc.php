	...
	function send( $msg ) {
		...
		if ( is_array( $msg ) ) {
			...
			$r = new XML_RPC_Response(
				0, $this->xmlrpcerr[ 'multicall_recursion' ], $this->xmlrpcstr[ 'multicall_recursion' ]
			);
...
class XML_RPC_Response {
	...
	function XML_RPC_Response( $val, $code = 0, $fstr = '' ) {
		...
		if ( $code != 0 ) {
			...
			error_log(
				"Invalid type '" . gettype( $val ) . "' (value: $val) passed to XML_RPC_Response.  Defaulting to empty value."
			);
...
class XML_RPC_Message extends CI_Xmlrpc {
	...
	function parseResponse( $fp ) {
		...
		if ( strncmp( $data, 'HTTP', 4 ) == 0 && ! preg_match( '/^HTTP\/[0-9\.]+ 200 /', $data ) ) {
			...
			$r      = new XML_RPC_Response(
				0, $this->xmlrpcerr[ 'http_error' ], $this->xmlrpcstr[ 'http_error' ] . ' (' . $errstr . ')'
			);
		...
		if ( $this->xh[ $parser ][ 'isf' ] > 1 ) {
			...
			$r = new XML_RPC_Response(
				0, $this->xmlrpcerr[ 'invalid_return' ],
	...
	function closing_tag( $the_parser, $name ) {
		...
		switch ( $name ) {
			...
			case 'ARRAY':
				...
				$this->xh[ $the_parser ][ 'value' ] = ( ! isset( $cur_val[ 'values' ] ) ) ? array()
					: $cur_val[ 'values' ];
